...
- [ ] **A.5.1 `scripts/ax_product_run.py` 신규 작성**
  ...
  - 동작:
    1. `run_id = uuid4()` 발행
    2. `shutil.copytree(fixture_root, $TMPDIR/ax-run-<run_id>)` — fixture 는 **plain 디렉토리** (git repo 아님). 복사본은 run_dir 로 격리
    3. run_dir 에서 `git init + add + commit "seed"` — 태스크별 커밋을 내기 위한 로컬 git repo 확보 (fixture 원본과 독립)
    4. `src/claude.py:call(cwd=run_dir, stdout_path=.harness/runs/<run_id>.ndjson, ...)` 호출 — `plugin_dir=<abs>/plugin`, `setting_sources="project,local"`, `output_format="stream-json"`, `permission_mode="acceptEdits"`, `include_hook_events=True`
    5. stream-json stdout 은 `stdout_path` 로 자동 기록 (드라이버가 별도 파싱 안 함)
    6. try/finally 로 `shutil.rmtree(run_dir)` — fixture 원본은 건드리지 않음 (복사만 했으므로)
    7. **cleanup 검증**: run_dir 가 실제로 제거됐는지 + fixture 원본에 `.git` 이 안 생겼는지 확인. 이상 시 stderr WARN
    8. `--keep` 옵션 시 run_dir 보존 (디버깅용)
...
### A.6 fixture — 격리 static 템플릿 + copytree 격리

fixture 원본은 **plain 디렉토리** (git repo 아님). driver 가 매 run `shutil.copytree` 로 `$TMPDIR/ax-run-<run_id>` 에 복사본 생성 + `git init + commit "seed"` 로 격리 git 상태 부여. 자식 세션 cwd = run_dir. 종료 시 `shutil.rmtree` 로 제거. (git worktree 를 택하지 않은 이유: fixture 를 moomoo-ax 의 embedded git 으로 만들면 gitlink 문제가 생기고, submodule 도 과잉. copytree 가 가장 단순하며 fixture 원본 보존도 확실)

- [ ] **A.6.1 `labs/ax-implement/input/static-nextjs-min/` 템플릿 신규**
  - 최소 Next.js + TypeScript + ESLint + i18n + DESIGN_SYSTEM 토큰
  - `versions/v0.3-fixture/plan.md` 내장 — T-001~T-003 수준 2~3개 태스크 (예: "T-001 Button 컴포넌트 신규 — primary/secondary variant", "T-002 유닛 테스트", "T-003 page.tsx 에 Button 시연")
  - `ARCHITECTURE.md` + `DESIGN_SYSTEM.md` 최소
  - `package.json` scripts: lint / typecheck / test / build 전부 존재
  - **plain 디렉토리로 유지** (moomoo-ax 에서 일반 파일로 트래킹). `.git` 없음
  - 템플릿은 readonly 운영 — 이후 템플릿 원본에 수정 금지
- [ ] **A.6.2 driver 의 copytree 격리 (clean-up 용)**
  - A.5.1 step 2, 6 참조
  - run_dir 경로: `$TMPDIR/ax-run-<run_id>/`
  - run 실패해도 try/finally 로 `shutil.rmtree` best-effort
...
| R5 | `shutil.copytree` cleanup 실패로 tmp 디렉토리 누적 | 낮 | run_id 기반 고유 경로 + try/finally rmtree + `--keep` 플래그로 선택적 보존 |
...
| R12 | fixture 원본이 run 으로 오염 (git 상태 / 파일 변경) | 해소 | `shutil.copytree` 로 복사본에서만 작업. fixture 는 plain 디렉토리로 유지. cleanup 검증에서 fixture 에 `.git` 안 생겼는지 확인 |
...
- 2026-04-13: **Phase A 구현 완료** — SKILL.md (A.1), scripts 4개 (A.2), references/templates 이관 (A.3), subagents 3개 포팅 (A.4), `scripts/ax_product_run.py` 드라이버 (A.5), fixture 템플릿 (A.6) 생성. 드라이버는 **`git worktree` 대신 `shutil.copytree + git init`** 로 구현 — fixture 를 plain 디렉토리로 두고 moomoo-ax 에서 자연스럽게 트래킹 (submodule/gitlink 회피). Phase A.7 실 run 은 `npm install` 필요하여 후속.
