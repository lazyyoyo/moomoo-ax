...
- **루프가 개선하는 건 "산출물"이 아니라 "스크립트"** — CPS를 잘 쓰는 방법, PRD를 잘 쓰는 방법 자체를 최적화
...
| 버전 | 목표 |
|------|------|
| **v0.2** | 루프 엔진 + CPS 작성 실험 + Supabase 로그 + 대시보드 |
| v0.3 | define(PRD) 루프 |
| v0.4 | design 루프 + 시각 eval |
| v0.5 | implement 루프 + 정적 게이트 |
| v0.6~0.9 | 실전 적용 + 강화 |
| v1.0 | 출시 + 남편 공유 |
...
- 루프 엔진(loop.py)은 범용. 실험별 차이는 program.md + rubric.yml로 표현
- subprocess.run으로 Claude 호출. 외부 의존성 최소화
- 유저: yoyo / jojo
