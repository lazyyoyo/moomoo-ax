...
Phase A가 `versions/undefined/`에 산출물을 만들면, Phase C가 spec in-place 갱신을 수행한다. 브랜치/워크트리 생성은 ax-build 소관 (Phase B는 ax-build로 이동됨).
...
> Phase C는 `versions/undefined/scope.md`를 대상으로 작업한다. 브랜치/워크트리 생성은 ax-build에서 수행.
