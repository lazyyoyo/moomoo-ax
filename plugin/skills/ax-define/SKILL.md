...
| **출력 (플러그인 v0.4)** | Phase A 산출물 3개(`intake.md` / `interview.md` / `scope.md`) + Phase C `docs/specs/` in-place 갱신 + `BACKLOG.md` inbox 정리. scope 확정 후 ax-build로 넘긴다. |
...
Phase A가 `versions/undefined/`에 산출물을 만들면, Phase C가 spec in-place 갱신을 수행한다. 브랜치 생성은 ax-build 소관 (Phase B는 ax-build로 이동됨). v0.8부터 worktree는 사용하지 않으며 단일 `version/vX.Y.Z` 브랜치에서 병렬 빌드가 돈다.
...
> Phase C는 `versions/undefined/scope.md`를 대상으로 작업한다. 브랜치 생성은 ax-build에서 수행 (v0.8: 단일 `version/vX.Y.Z` 브랜치).
