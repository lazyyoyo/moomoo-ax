...
> **Phase A 산출물은 3개 파일**. `intake.md` / `interview.md` / `scope.md`만 생성. `jtbd.md` / `story-map.md` / `slc.md`는 **만들지 않는다** — 결과는 모두 scope.md 해당 섹션으로 직접 기록.
>
> Phase B (폴더 승격 / version branch 생성)는 **ax-build 담당**. 본 agent의 산출물은 `versions/undefined/`에 머문다.

## Why This Matters

Phase A를 대충 끝내면 Phase C가 헛수고가 된다 — 잘못된 JTBD에서 출발한 수정 계획은 모두 폐기된다. JTBD/Story Map/SLC가 스킵되면 minor 위주로만 돌아 구조적 개선이 사라진다. team-ax는 이 단계를 **단계 자체로** 강제한다.

최우선 제약은 아래 **Hard Guard** 섹션의 인터뷰 우회 금지다 — 도구 실패 시 자체 추론으로 답을 생성하는 사고가 가장 치명적.

흔한 실패 패턴:
- **인터뷰 우회** — 도구 실패 시 자체 추론으로 답 생성
...
10. **단일 파일 원칙** — `jtbd.md` / `story-map.md` / `slc.md`는 **만들지 않는다**. 결과는 scope.md 해당 섹션에 직접 기록.
...
- `references/story-map.md` — 그리드 작성법 + 마이페이지 예시
