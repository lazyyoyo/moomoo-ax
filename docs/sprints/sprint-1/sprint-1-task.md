...
- [ ] T0.1 — `plugin/skills/ax-define/` 생성
- [ ] T0.2 — `plugin/skills/ax-define/references/` 생성
- [ ] T0.3 — `plugin/skills/ax-define/templates/` 생성
- [ ] T0.4 — `plugin/skills/ax-review/` 생성
- [ ] T0.5 — `plugin/skills/ax-review/references/` 생성
- [ ] T0.6 — `plugin/agents/` 확인 (존재)
- [ ] T0.7 — 빈 `SKILL.md` placeholder (ax-define + review 두 곳, T4·TR에서 채움)
...
- [ ] **T1.1 — `references/jtbd.md`** (Jobs To Be Done 프레임워크)
...
- [ ] **T1.6 — `references/docs-structure.md`** (BACKLOG / scope / CHANGELOG 3-문서)
...
- [ ] **T3.1 — `plugin/agents/product-owner.md`**
...
- [ ] **TR.1 — `plugin/skills/ax-review/SKILL.md`** (스킬 본체 — Claude/Codex 공용 정본)
  - frontmatter:
    - `name: ax-review`
...
- [ ] **TR.2 — `plugin/skills/ax-review/references/doc-checklist.md`** (v0.1 구현 타입)
...
- [ ] **TR.3 — `plugin/skills/ax-review/references/code-checklist.md`** (v0.1 stub)
  - 한 줄 문서: "구현 리뷰 체크리스트. 후속 스프린트(ax-build 도입 시)에서 작성."
  - SKILL.md가 이 타입 호출 시 "미구현" 안내 후 중단.

- [ ] **TR.4 — `plugin/skills/ax-review/references/pr-checklist.md`** (v0.1 stub)
  - 한 줄 문서: "PR 리뷰 체크리스트. 후속 스프린트(ax-deploy 도입 시)에서 작성."
  - SKILL.md가 이 타입 호출 시 "미구현" 안내 후 중단.

- [ ] **TR.5 — `plugin/scripts/install-local-skills.sh`** (codex·claude 스킬 동기화)
...
- [ ] **T5.1 — 용어 일관성**: "제품 버전" vs "플러그인 버전" 구분이 모든 문서에서 흐트러지지 않는지
- [ ] **T5.2 — 경로 교차검증**: ax-define SKILL.md와 review SKILL.md가 참조하는 `references/*.md` / `templates/*.md` / `agents/*.md` 경로가 실제 파일과 일치
- [ ] **T5.3 — Phase B 부재 일관성**: 모든 산출물 저장 위치가 `versions/undefined/`임. `versions/vX.Y.Z/` / `cycle/X.Y.Z` / worktree 언급은 "v0.2 예정" 주석 있을 때만
- [ ] **T5.4 — 책임 경계**: plan/write는 analyst 전담, review는 `$ax-review doc` 스킬 전담(codex 위임). analyst에 쓰기 권한, review 스킬은 codex 호출만.
- [ ] **T5.5 — 강제 장치 4종 체크**: `review/references/doc-checklist.md`에 4종이 모두 들어가 있음
- [ ] **T5.6 — ax-review 스킬 타입 stub**: `code` / `pr` 타입이 호출 시 "미구현" 안내 후 중단 동작 (인자 파싱은 정상)
- [ ] **T5.7 — 설치 스크립트 검증**: `install-local-skills.sh` 실행 시 `~/.codex/skills/ax-review/` + Claude 캐시 경로 갱신 확인. `ls` 결과 SKILL.md + references/ 존재.
- [ ] **T5.8 — sprint-1-plan과의 부합**: plan 문서의 "범위 / 비범위 / 성공 기준"이 실제 구현에 반영됨
...
- [ ] T0 완료 (스캐폴딩 — ax-define + review 양쪽)
- [ ] T1 완료 (references 6개)
- [ ] T2 완료 (ax-define templates)
- [ ] T3 완료 (에이전트 2개: product-owner, analyst)
- [ ] TR 완료 (ax-review 스킬 본체 + doc-checklist + code/pr stub + install-local-skills.sh + AGENTS.md)
- [ ] T4 완료 (ax-define SKILL.md)
- [ ] T5 PASS (7개 self-review 항목)
- [ ] T6 릴리즈 (v0.1.0 태그)
