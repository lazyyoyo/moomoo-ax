...
| train.py | 프롬프트, hooks, eval 설정 | `agents/*.md`, `harness/`, `hooks/` | 버전마다 조정 |
| prepare.py | 레벨 1 지표 (eval 함수) | `scripts/metrics.py` | 지표 추가 시만 변경 |
| results.tsv | 버전 리포트 | `meta/reports/vN.md` | 자동 생성 |
...
def collect_version_metrics(logs_dir, version_range):
    """
    버전 기간 내 모든 iteration 로그에서 레벨 1 지표 집계.
    출력: vN.md 리포트 + metrics.json (대시보드용)
    """
```
...
## 버전 리포트 (results.tsv 역할)

매 버전 릴리즈(또는 N회 ax-loop 실행) 후 자동 생성.
...
- **정기**: 버전 릴리즈 후 (v0.1 → v0.2 등 버전 전환 시점)
...
```
① 지표 수집
   scripts/metrics.py → .harness/logs/ 분석 → 버전 리포트 초안
