...
#### meta-loop: AX 시스템 진화 (버전 단위)

시스템 자체가 팀을 돕고 있는지 평가. 버전 단위로 지표 수집 → 진단 → 변경 → 검증.
상세는 `meta-loop.md` 참조.

| auto-research 요소 | meta-loop 매핑 | 설명 |
|-------------------|---------------|------|
| program.md | program.md, agents/*.md | 오너가 시간 두고 개선 |
| train.py | agents/, harness/, hooks/, gate 설정 | 버전마다 조정 |
| prepare.py | 레벨 1 지표 (scripts/metrics.py) | 토큰량, 개입횟수, 수용율, 재작업횟수, 게이트 통과율 |
| results.tsv | 버전 리포트 (meta/reports/vN.md) | 변경 사항 + 지표 + keep/revert |
...
meta-loop의 eval 함수. 상세 지표 목록, 수집 방법, 버전 리포트 구조는 `meta-loop.md` 참조.
...
**실행 시점**: meta-loop(버전 단위)에서 하네스 점검 시. 매 기능 빌드마다 돌리는 것이 아니라, 하네스 설정을 변경한 뒤 검증 용도.
