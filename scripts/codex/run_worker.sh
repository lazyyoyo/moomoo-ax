...
usage() {
  cat <<'EOF'
run_worker.sh --role <executor|reviewer> --task-id <id> --cd <dir> --task-file <file> --log-dir <dir> [options]

Stable Codex worker wrapper for v0.4 Track B.
...
python3 "$REPO_ROOT/src/codex_worker.py" \
  --role "$ROLE" \
  --task-id "$TASK_ID" \
  --log-dir "$LOG_DIR" \
...
echo "[codex-worker] result=$RESULT_JSON_PATH"
